...
ALWAYS do any fix or enhancements in a separate branch other than main (never directly on main). Once all development is done, add relevant tests (unit and E2E to the test suite); run the tests till everything is green. Once everything passes, then you ask for my permission to merge to main. Once merged to main, run all tests again to ensure no regressions. Post that update all the documentation (README, CLAUDE.MD and both Technical Documentation and User Guide as required). Then ask for my explicit approval for PUSH to remote GITHUB. Then do cleanup - i.e. delete the newly created branch on which the fix / development was done after ensuring that deleting it is safe. If it is a minor release, bump up Release tag by 0.0.1; if major then bump up to next number (e.g from 2.0.0 to 3.0.0)