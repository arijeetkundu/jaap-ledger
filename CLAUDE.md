...
```bash
# Run locally (app uses fetch() for data.json/poornima.json, so file:// won't work)
...
- **IndexedDB** (`jaap-ledger-db`, `DB_VERSION` in `app.js`) holds practice data only, across 4 object stores: `ledger` (live entries), `ledger-backups` (latest auto-backup, overwritten each save), `meta` (one-time seed-from-`data.json` guard), `sankalpa` (single vow record). The upgrade handler is additive-only — bump `DB_VERSION` and add a store only if needed; never destroy existing stores.
...
**Bootstrap sequence** (`initApp()`, async IIFE in `app.js`): request persistent storage (best-effort) → fetch `poornima.json` (fallback full-moon calendar; primary Poornima detection is keyword-based via `hasExplicitPoornima()`) → `loadLedgerFromDB()` (live store → latest backup → empty) → seed from `data.json` only on a never-before-seeded empty ledger (`meta.ledgerSeeded` guard — an emptied ledger is never re-seeded) → `ensureRecentEntriesExist(7)` backfills null placeholders for the last 7 days → persist + fresh backup → `renderToday()`. `renderToday()` is the single entry point that re-renders Today Card + Reflection Summary + Ledger List; call it after every mutation rather than patching the DOM piecemeal.
...
- `data.json` on this machine carries an intentional local modification that must never be staged or committed — always exclude it explicitly (`git add <specific files>`, never `git add -A`/`.`).
