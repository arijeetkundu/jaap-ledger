...
- **Lifetime average** — a quiet per-day figure beside the Ledger heading, over the days you actually practised rather than every day on the calendar. Informational only; it feeds no prediction.
...
- **Knowing it worked.** A "Last synced" line above the button shows the local date and time of the last sync that genuinely completed — written only after a committed push or a pull that was read *and* applied, never on an attempt and never on a failure. It stays empty until something has actually synced.
