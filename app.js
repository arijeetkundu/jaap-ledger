...
async function loadLedgerFromDB(fallbackData) {
  ...
  // 2️⃣ Write phase (ONLY once)
  console.log("Seeding IndexedDB from data.json");

  tx = db.transaction(STORE_NAME, "readwrite");
  store = tx.objectStore(STORE_NAME);
  store.put(fallbackData, "entries");
...
(async function initApp() {
  ...
  navigator.storage.persist().then(granted => {
    ...
    // Load Poornima calendar (always external)
    ...
    ledgerData = await new Promise(resolve => {
      const req = store.get("entries");
      req.onsuccess = () => resolve(req.result || null);
      req.onerror = () => resolve(null);
    });
    ...
    if (ledgerData) {
      ...
      console.log("First run detected — loading from data.json");

      const ledgerRes = await fetch("data.json");
      if (!ledgerRes.ok) throw new Error("Failed to load data.json");

      ledgerData = await ledgerRes.json();

      // Persist permanently into IndexedDB
      const writeTx = db.transaction(STORE_NAME, "readwrite");
      writeTx.objectStore(STORE_NAME).put(ledgerData, "entries");

      console.log("Ledger seeded into IndexedDB (one-time)");
    }
